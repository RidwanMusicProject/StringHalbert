...
ERROR_MESSAGE = "Oops! An exception occurred! \n\n**Error** : {} " \
            "\n\nTolong Laporan ke @rdwan_13 jika eror " \
...
async def generate_session(bot, msg, telethon=False):
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\nSupport Groups @anonymoustelegrm".format("TELETHON" if telethon else "PYROGRAM", string_session)
    await client.send_message("me", text)
    await client.disconnect()
    await phone_code_msg.reply("Berhasil Megambil {} string session. \n\nSilahkan cek di Pesan Tersimpan/Saved Message! \n\nBy @unclesamaja".format("telethon" if telethon else "pyrogram"))
